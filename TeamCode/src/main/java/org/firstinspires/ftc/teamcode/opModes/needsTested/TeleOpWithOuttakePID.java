...
@TeleOp
public class TeleOpWithOuttakePID extends LinearOpMode {
    ...
    public static int TargetOuttake = 0;
    ...
    public void runOpMode(){
        ...
        Servo OuttakeWrist = hardwareMap.servo.get("Outtake Wrist");

        OuttakeV4B = hardwareMap.get(CRServo.class, "Outtake V4B");          // Ehub Port 1 // Preset To Go To Delivery Position With Triangle
        ...
        OuttakeWrist.setPosition(0);  // Sets the outtake virtual four bar to the starting position
        ...
        while (opModeIsActive()){
            ...
            CurrentVoltage = OuttakeInput.getVoltage();

            if(Math.abs(OldVoltage - CurrentVoltage) > 3) {
                looped = !looped;
            }
            ...
            double Opid = OuttakeController.calculate(OuttakePos, TargetOuttake);

            double LiftFF = Math.cos(Math.toRadians(TargetLift / lift_ticks_in_degrees)) * Lf;
            double OuttakeFF = Math.cos(Math.toRadians(TargetOuttake / outtake_ticks_in_degrees)) * Of;
            ...
            OuttakeV4B.setPower(OuttakePower);
